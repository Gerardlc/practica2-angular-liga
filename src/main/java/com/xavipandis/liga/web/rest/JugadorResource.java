...
public class JugadorResource {
    ...
    //jugadores por asistencias Ex2
    @RequestMapping(value = "/topAsists/{asistencias}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Jugador>> topAsists(@PathVariable Integer asistencias, Pageable pageable)
        throws URISyntaxException {
        Page<Jugador> page = jugadorRepository.topAsists(asistencias, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/jugadors/topAsists");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
